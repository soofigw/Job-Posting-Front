...
import { loadSession } from "../../caracteristicas/autenticacion/authService.js";
...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        if (!token) return;
        if (!actor) return;
        if (actor.type !== "candidate") return;

        const cargarFavoritos = async () => {
            try {
                const res = await api.get(`/favorites?page=1&limit=200`, {
                    headers: { Authorization: `Bearer ${token}` }
                });

                if (res.data && res.data.status === "ok" && Array.isArray(res.data.items)) {
                    const ids = res.data.items
                        .map((it) => it?.job?.job_id || it?.job?._id || it?.job_id || it?._id)
                        .filter(Boolean)
                        .map((x) => String(x));
    ...
    const handleToggleFavorito = async () => {
        ...
        if (actor.type !== "candidate") {
            alert("🧐 Solo los candidatos pueden guardar favoritos.");
            return;
        }
        ...
        const yaEsFav = favoriteIds.has(key);
        ...
        try {
            if (yaEsFav) {
                ...
                const res = await api.post(`/favorites/${jobId}`, null, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            ...
            if (err.response && (err.response.status === 401 || err.response.status === 403)) {
                ...
                alert("Error de conexión guardando favorito.");
            }
        } finally {
            setFavLoading(false);
        }
    ...
    const selectedKey = selectedJob ? String(selectedJob.job_id || selectedJob._id) : "";
    const selectedIsFav = selectedKey ? favoriteIds.has(selectedKey) : false;
                    ...
                    {loading ? ( <div style={{padding:40, textAlign:'center', color:'#888'}}>Buscando...</div> ) : vacantes.length === 0 ? ( <div style={{padding:40, textAlign:'center', color:'#888'}}>No hay resultados 😢</div> ) : (
                        <>
                            ...
                            {vacantes.map((job) => {
                                ...
                                const jobKey = String(job.job_id || job._id);
                                const isFav = favoriteIds.has(jobKey);
                                    ...
                                    <div key={job._id || job.job_id} className={`dash-card ${isSelected ? "active" : ""}`} onClick={() => setSelectedJob(job)}>
                                        ...
                                        <div className="dash-footer">
                                            <div className="dash-badges">
                                                ...
                                                {isFav && (
                                                    <span className="dash-chip" style={{ border: "1px solid #FF5A5F", background: "#fff", color: "#FF5A5F", fontWeight: 800 }}>
                                                        <FaRegBookmark style={{ marginRight: 6, position: "relative", top: 1 }} />
                                                        Favorito
                                                    </span>
                                                )}
                                            </div>
                                ...
                                <div className="header-left">
                                    ...
                                    <div className="header-actions-row">
                                        ...
                                        <button
                                            ...
                                            disabled={favLoading}
                                            title={selectedIsFav ? "Quitar de favoritos" : "Guardar en favoritos"}
                                            style={{
                                                borderColor: selectedIsFav ? "#FF5A5F" : undefined,
                                                boxShadow: selectedIsFav ? "0 0 0 3px rgba(255,90,95,0.15)" : undefined,
                                                transition: "all .15s ease"
                                            }}
                                        >
                                            <FaRegBookmark
                                                style={{
                                                    color: selectedIsFav ? "#FF5A5F" : undefined,
                                                    transform: selectedIsFav ? "scale(1.06)" : "scale(1)",
